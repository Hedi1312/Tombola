...
export default function WinProbabilityAdmin() {
    const [probability, setProbability] = useState(0); // valeur réelle enregistrée
    const [manualInput, setManualInput] = useState(0); // valeur temporaire
    ...
    const saveProbability = async (value?: number) => {
        try {
            const finalValue = value !== undefined ? value : manualInput;
    ...
    return (
        <section className="min-h-screen flex flex-col items-center justify-start pt-16 px-4 md:px-6 bg-gray-50">
            <div className="w-full max-w-4xl mx-auto rounded-2xl bg-white p-6 md:p-8 shadow-md mb-12">
                ...
                <div className="my-15">
                    {/* Presets (changent directement la probabilité) */}
                    <div className="w-full max-w-4xl mx-auto grid grid-cols-3 gap-2 mb-6">
                        {presetValues.map((val) => (
                            <button
                                key={val}
                                onClick={() => saveProbability(val)} // sauvegarde immédiate
                                disabled={loading}
                                className={`px-4 py-2 rounded-lg font-bold border cursor-pointer ${
                                    probability === val
                                        ? "bg-blue-600 text-white"
                                        : "bg-gray-100 text-gray-700"
                                } hover:bg-blue-500 hover:text-white transition text-center disabled:opacity-50`}
                            >
                                {val}%
                            </button>
                        ))}
                    ...
                    {/* Champ manuel (requiert Enregistrer) */}
                    <form
                        onSubmit={(e) => {
                            e.preventDefault();
                            saveProbability(Number(manualInput));
                        }}
                    ...
                    >
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="px-4 py-2 mt-2 w-full sm:max-w-xs sm:mx-auto flex items-center justify-center gap-2 bg-green-700 text-white font-medium rounded-lg hover:bg-green-800 shadow-sm hover:shadow-md transition cursor-pointer disabled:opacity-50"
                        >
